...
## This code is adapted from Hay et al. 2021
library(tidyverse)
library(data.table)
library(coda)
library(MASS)
...
main_wd <- "/Users/caraebrook/Documents/R/R_repositories/virosolver_paper/"
main_wd2 <- "/Users/caraebrook/Documents/R/R_repositories/COVID-Ct-Madagascar/Mada-Ct-Distribute/"
setwd(main_wd)

source("code/plot_funcs.R")
source("code/priors.R")

#devtools::load_all("/Users/caraebrook/Documents/R/R_repositories/virosolver/")
library(virosolver)
#SAVE_PLOTS <- TRUE


#load Hay generic parameters for optim
parTab <- read.csv("/Users/caraebrook/Documents/R/R_repositories/virosolver_paper/pars/generic/partab_for_optim.csv")


#load mada data
load("/Users/caraebrook/Documents/R/R_repositories/COVID-Ct-Madagascar/Mada-Ct-Distribute/individual-Ct/mada.viral.fit.Rdata")
head(viral.fit)

#plot
p1 <- ggplot(viral.fit) + theme_bw() +
  geom_point(aes(x=days_since_infection_onset, y=Ct), alpha=.4) +
  scale_y_reverse() + coord_cartesian(xlim=c(0,50), ylim=c(40,10))
p1

## Ct value plateaus at 38 in Hay BWH data, 33 in Hay NH data, 33 our in Mada data
#par_key <- c("viral_peak"="Ct[peak]","obs_sd"="sigma[obs]","t_switch"="t[switch]","prob_detect"="p[addl]","prob"="1/(1+e^(-pi))",
#"R0" ="R[0]", "t0" ="t[0]", "incubation"="1/sigma", "infectious" = "1/gamma", "level_switch" = "C[switch]")
parTab[parTab$names == "level_switch","values"] <- 36 #Modal Ct value at a = teclipse (0) + tpeak (5) + tswitch (~9-14)
parTab[parTab$names == "viral_peak","values"] <- 22 #Modal Ct value at peak viral load
parTab[parTab$names == "true_0", "values"] <- 37 #Limit of detection of Ct value
parTab[parTab$names == "intercept", "values"] <- 37 #Ct value at time of infection
ct_lower <- range(viral.fit$Ct,na.rm=TRUE)[1]
ct_upper <- 35
...
## load Borremans detection probs for fitting
to_fit_probs <- read_csv("/Users/caraebrook/Documents/R/R_repositories/virosolver_paper/data/borremands_urt_pos.csv")
...
head(line_dat)
names(fitted_dat)[names(fitted_dat)=="age"]<- "t"

line_dat <- line_dat %>% left_join(fitted_dat)

hists <- hists %>% left_join(fitted_dat)
...
p1 <-  ggplot(data=line_dat) +
  geom_point(data=subset(viral.fit, days_since_infection_onset<55), 
             aes(x=days_since_infection_onset, y = Ct), alpha=.2) +
  geom_violin(data=hists, aes(x=t, y=value, group=t, fill=prob),scale="width",
              draw_quantiles = c(0.025,0.5,0.975), alpha=.8) +
  ...
  geom_line(data=tmp_vl_melted %>% filter(samp %in% sample(unique(tmp_trajs_melted$samp), 25)),
            aes(x=t,y=vl,group=samp),alpha=0.75,size=0.1, col="cornflowerblue")+
  scale_fill_gradient(name="Proportion detectable:", low="blue", high="red",
                      ...
                      guide=guide_colorbar(direction="horizontal",title.position="top",barwidth=5,barheight=1,
                                           ticks=FALSE)) +
  geom_line(aes(x=t, y=mean_load), col="cornflowerblue",size=1.2) +
...
p2 <- ggplot(data=fitted_dat) +
  ...
  geom_line(aes(x=t, y=prob, col=prob),size=1.2) +
...
p3 <- ggplot(all_pars_melted %>% filter(par %in% c("viral_peak","obs_sd",
  ...
  geom_density(aes(x=value), fill="cornflowerblue",alpha=0.5) +
  ylab("Prior density") +
  xlab("Value") +
...
ggsave(file = "/Users/caraebrook/Documents/R/R_repositories/COVID-Ct-Madagascar/Mada-Ct-Distribute/fig-plots/FigS3.png",
    ...
    parTab$values <- pars1
    write_csv(parTab,"/Users/caraebrook/Documents/R/R_repositories/COVID-Ct-Madagascar/Mada-Ct-Distribute/common_pars/partab_fitted_mada.csv")
