...
#get posterior distribution plot
...
chain_plot <- chain_plot[,c(1:9,ncol(chain_plot))]                      
...
chain_plot$district <- "Analamanga"
...
add <- chain_plot[1,]
add$chain <- "4"
#add$name <- as.character(NA)
add$value <- as.double(0)
chain_plot <- rbind(chain_plot, add)
#chain_plot$chain <- as.factor(chain_plot$chain)

p_posteriors_seir <- ggplot(chain_plot) +
  geom_density(aes(x=value,fill=chain),alpha=0.5) +
  facet_wrap(~name,labeller=label_parsed,scales="free", nrow=2) +
  scale_y_continuous(expand=c(0,0)) + 
  scale_fill_manual(values=colz) +
  theme_bw() +
  theme(strip.text = element_text(size=12), 
        panel.grid = element_blank(), legend.position = c(.93,.25),
...
p_posteriors_gp <- ggplot(chain_plot) +
  geom_density(aes(x=value,fill=chain),alpha=0.5, show.legend = F) +
...
FigS6 <- cowplot::plot_grid(p_posteriors_gp, p_posteriors_seir, nrow=2, ncol=1, rel_heights = c(1,2), labels=c("A.", "B."))

setwd(paste0(homewd, "/fig-plots/"))

ggsave(file = "FigS6.png",
       plot = FigS6,
       units="mm",  
       width=80, 
       height=80, 
